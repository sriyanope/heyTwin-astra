...
export function createApp({ env = loadEnv(), provider = createProvider(env), now = Date.now } = {}) {
  ...
  function sweep() { for (const [id, session] of sessions) if (session.expires <= now()) sessions.delete(id); }
    ...
    try {
      ...
      if (request.method === 'POST' && pathname.startsWith('/api/')) {
        ...
        if (pathname === '/api/analyze-garment') {
          ...
          sessions.set(garment_id, { analysis, expires: now() + sessionTTL });
          return sendJson(response, 200, analysis);
        }
        if (!['/api/confirm-garment', '/api/recommend-outfits', '/api/discard-garment'].includes(pathname)) throw failure(404, 'NOT_FOUND', 'Not found.');
        ...
        const outfits = result.outfits.map((outfit, index) => {
          ...
          used.add(item.id);
          return { outfit_id: `${payload.garment_id}-${index}`, name: outfit.name, items: [
            { item_id: payload.garment_id, ownership: 'user_item', ...confirmed, image_ref: 'user_upload', source_state: 'live' },
            { item_id: item.id, ownership: 'suggested_item', category: item.category, description: item.description, colour: item.colour, pattern: item.pattern, image_ref: item.image_ref, image_label: 'Illustrative pairing', source_state: 'live' }
