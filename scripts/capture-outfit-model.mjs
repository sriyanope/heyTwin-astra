...
const report = JSON.parse(await fs.readFile('test-results/outfit-live-results.json', 'utf8'));
...
try {
  ...
  await page.evaluate(async ({ asset }) => {
    document.body.innerHTML = '<main style="max-width:920px;margin:auto;padding:24px;font-family:Arial,sans-serif;color:#162b48"><h1 style="font-size:28px">Simplified outfit in 3D</h1><p>White short-sleeve T-shirt + indigo straight-leg jeans</p><p>OpenAI-assisted garment parameters · Fixed neutral mannequin · Not a fit prediction</p><model-viewer camera-controls shadow-intensity="1" exposure="1" style="display:block;width:100%;height:710px;background:#f6eedc;border:2px solid #162b48;border-radius:16px"></model-viewer><p>Live OpenAI text response, built locally into a GLB. Patterns and fine details are simplified.</p></main>';
  ...
  for (const [name, theta] of [['front', 0], ['side', 90], ['back', 180]]) {
    ...
    await page.screenshot({ path: `test-results/openai-model-${name}.png` });
  }
  ...
  const result = { asset_url: asset, loaded: after.loaded, drag_rotates: true, wheel_zooms: true, page_errors: errors, screenshots: ['front', 'side', 'back'].map(view => `test-results/openai-model-${view}.png`) };
  await fs.writeFile('test-results/openai-model-browser.json', JSON.stringify(result, null, 2));
