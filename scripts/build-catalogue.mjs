...
for (const item of catalogue) {
  ...
  writeFileSync(new URL(`../public${item.image_ref}`, import.meta.url), `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 400" role="img" aria-label="Illustration: ${item.description}"><title>${item.description} — illustrative</title><ellipse cx="160" cy="355" rx="89" ry="9" fill="#28251d" opacity=".07"/><g fill="${item.fill}" stroke="#55584f" stroke-width="1.5" stroke-linejoin="round">${shape}</g></svg>`);
}
