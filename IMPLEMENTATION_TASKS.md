...
Suggestion-image generation starts once when confirmed pairings appear, never on rerenders; failed jobs require manual retry. Outfit previews and 3D remain explicit actions. Original uploads stay transient; generated images and models persist under `data/generated`. Cached output identity includes generation settings and relevant attributes. Reuploading the same photo after refresh can recover matching cached assets without another paid job.
