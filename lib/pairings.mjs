...
export function garmentAttributes(item) {
  ...
  return { category: item.category, garment_type, colour: item.colour, pattern: item.pattern, material, silhouette, length, details };
}

export function pairingContext(session, item) {
  const attributes = garmentAttributes(item);
  return {
