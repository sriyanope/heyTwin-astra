...
export function createGenerationService({ env = {}, fetchImpl = fetch, now = Date.now, storageDir = DEFAULT_DIR } = {}) {
  ...
  function settings(kind) {
    ...
    const result = { prompt_version: 2, model: env.IMAGE_MODEL || 'gpt-image-1.5', quality: env.IMAGE_QUALITY || 'medium', size: kind === 'preview' ? '1024x1536' : '1024x1024' };
  ...
  function imageKey() {
    ...
    try { if (new URL(base).origin === 'https://api.openai.com') return env.VISION_MODEL_API_KEY; } catch {}
    return undefined;
  }
  ...
  function modelName() { return env.MODEL_3D_MODEL || env.VISION_MODEL || 'gpt-5.6-terra'; }
  ...
  async function generateImage(key, kind, context, reference) {
    ...
    const endpoint = kind === 'preview' ? 'https://api.openai.com/v1/images/edits' : 'https://api.openai.com/v1/images/generations';
    try {
      let options;
      if (kind === 'preview') {
        ...
        form.set('model', optionsFor.model); form.set('prompt', prompt(kind, context)); form.set('size', optionsFor.size); form.set('quality', optionsFor.quality);
        form.append('image[]', new Blob([original.data], { type: original.contentType }), 'reference.png');
        const suggestion = await fetchGenerated(context);
        if (suggestion) form.append('image[]', new Blob([suggestion.data], { type: suggestion.contentType }), 'suggestion.png');
        options = { method: 'POST', redirect: 'error', signal: AbortSignal.timeout(Number(env.IMAGE_TIMEOUT_MS) || 90000), headers: { authorization: `Bearer ${apiKey}` }, body: form };
      } else { const optionsFor = settings(kind); options = { method: 'POST', redirect: 'error', signal: AbortSignal.timeout(Number(env.IMAGE_TIMEOUT_MS) || 90000), headers: { authorization: `Bearer ${apiKey}`, 'content-type': 'application/json' }, body: JSON.stringify({ model: optionsFor.model, prompt: prompt(kind, context), size: optionsFor.size, quality: optionsFor.quality }) }; }
